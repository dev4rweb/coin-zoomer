...
const SimpleTable = () => {
    ...
    const coinsMarketCap = useSelector(state => state.leaderCoins.leadersMarketCap)
    ...
    useEffect(() => {
        // console.log('Simple Table', coinsMarketCap)
    }, [coinsMarketCap]);
        ...
        return (
            <Table className={s.simpleTable} striped hover responsive variant="dark">
                ...
                <tbody>
                {
                    ...
                    /*coins.map((i, index) =>
                        <SimpleTableItem key={i.id} data={i} index={index}/>)*/
                    coinsMarketCap && coinsMarketCap.data.map((i, index) =>
                        <SimpleTableItem key={i.id} data={i} index={index}/>)
                }
                </tbody>
            </Table>
